...
from src.utils_data import load_Rennes_flow_data, preprocess_rennes_data, local_dataset, normalize_center_reduce
...
with open(PATH_EXPERIMENTS / 'test.txt', 'w') as f:
    with contextlib.redirect_stdout(src.config.Tee(f, sys.stdout)):
        ...
        df_PeMS = load_Rennes_flow_data()
        df_PeMS, meanstd_dict = preprocess_rennes_data(df_PeMS, 1,
