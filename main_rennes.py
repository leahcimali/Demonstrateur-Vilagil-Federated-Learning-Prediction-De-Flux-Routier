...
import src.config
from src.utils_data import load_Rennes_flow_data, preprocess_rennes_data, local_dataset
...
PATH_EXPERIMENTS = Path("experiments_rennes") / params.save_model_path
...
with open(PATH_EXPERIMENTS / "train.txt", 'w') as f:
    with contextlib.redirect_stdout(src.config.Tee(f, sys.stdout)):
        ...
        df_Rennes = load_Rennes_flow_data()

        df_Rennes, meanstd_dict = preprocess_rennes_data(df_Rennes, params.time_serie_percentage_length,
                                                        None, params.init_node, params.n_neighbours,
                                                        params.smooth, params.center_and_reduce,
                                                        params.normalize, params.sort_by_mean)

        prepare_training_configs(config_file_path, PATH_EXPERIMENTS, params, df_Rennes)

        datadict = local_dataset(df=df_Rennes,
